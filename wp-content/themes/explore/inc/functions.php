...
function explore_scripts_styles_method() {
   ...
   // enqueueing explore theme custom js file
	wp_enqueue_script( 'explore-custom', EXPLORE_JS_URL. '/explore-custom' . '.js', array( 'jquery' ) );
...
function explore_sidebar_layout() {
   ...
   $layout = get_theme_mod( 'explore_default_layout', 'right_sidebar' );
   ...
   elseif( is_page() ) {
      $layout = get_theme_mod( 'explore_pages_default_layout', 'right_sidebar' );
      if( $layout_meta != 'default_layout' && $layout_meta != '' ) {
         $layout = $layout_meta;
      }
   }
   ...
   elseif( is_single() ) {
      $layout = get_theme_mod( 'explore_single_posts_default_layout', 'right_sidebar' );
      if( $layout_meta != 'default_layout' && $layout_meta != '' ) {
         $layout = $layout_meta;
      }
   }
...
function explore_body_class( $classes ) {
	...
	$explore_default_layout = get_theme_mod( 'explore_default_layout', 'right_sidebar' );

	$explore_default_page_layout = get_theme_mod( 'explore_pages_default_layout', 'right_sidebar' );
	$explore_default_post_layout = get_theme_mod( 'explore_single_posts_default_layout', 'right_sidebar' );
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<header class="comment-meta comment-author vcard">
				<?php
					...
					printf( '<div class="comment-date-time">%1$s</div>',
						sprintf( __( '%1$s a las %2$s', 'explore' ), get_comment_date(), get_comment_time() )
					);
					edit_comment_link();
				?>
			</header><!-- .comment-meta -->
			...
			<section class="comment-content comment">
				<?php comment_text(); ?>
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Responder', 'explore' ), 'after' => '', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
