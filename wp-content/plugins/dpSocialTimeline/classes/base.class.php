					...
					foreach($timeline_feeds as $key) {
						...
						if($key->name == 'twitter_hash') { 
							...
							foreach($twitter_arr as $twitter) {
								if(trim($twitter) == "") { continue; }
								if($tw_string != "") { $tw_string .= ","; }
								$tw_string .= dpSocialTimeline_plugin_url('lib/search.php?q=%23'.urlencode(trim($twitter)));
							}
							$key->data = $tw_string; 
						}
						if($key->name == 'twitter_search') { 
							...
							foreach($twitter_arr as $twitter) {
								if(trim($twitter) == "") { continue; }
								if($tw_string != "") { $tw_string .= ","; }
								$tw_string .= dpSocialTimeline_plugin_url('lib/search.php?q='.urlencode(trim($twitter)));
							}
							$key->data = $tw_string; 
						}
